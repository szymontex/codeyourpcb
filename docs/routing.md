...
belong there. The canon states conditions a **board** has to meet; this states
three places where **this router** departs from the algorithm it is named after.
It fails the canon's own first entry condition, which asks that every quantity a
rule compares be in the world or derivable from it: two of these three compare
our code with a paper.
