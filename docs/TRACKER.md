...
- NEXT-ACTION: classify the remaining 3 clearance violations on led_blink with `cargo test --release -p cypcb-autoroute --test drc_report -- --ignored --nocapture`. If they are genuine trace-to-trace clearance, they belong to the router; if they are grid-boundary artifacts, the grid resolution or the pad-zone margin is the lead.
