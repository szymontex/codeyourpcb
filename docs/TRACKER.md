...
- **What those 25 symbols are settles the question.** Not libc - they are tree-sitter's own runtime plus our grammar: `ts_parser_new`, `ts_tree_root_node`, `ts_tree_cursor_goto_first_child`, `tree_sitter_cypcb` and 21 more. The C never got compiled. `which clang` in the build container returns nothing, so `cc` cannot target wasm32 at all, and **the wasm linker turns undefined symbols into imports instead of failing**, which is why the build reported success. The earlier measurement in this tracker - "the parser does build for wasm32, the note in Cargo.toml is wrong" - was true about `cargo build` and wrong about the artifact. A module that links is not a module that loads.
