...
- **Architectural finding:** `drill_size`, `connectivity`, `annular_ring` and `hole_to_hole` each construct their own `FootprintLibrary::new()` inside `check()`, so **no DRC rule can see a footprint defined in the .cypcb source**. `run_drc()` takes no library; threading one through reaches `score_board` and its 21 call sites. The alternative is for `BoardWorld` to own the effective library, which sync already builds. Deliberate call, not a drive-by.
- NEXT-ACTION: two stubs left. `silk_clearance.rs:26` needs silkscreen geometry, which the renderer already computes; `solder_mask_bridge.rs:27` needs per-pad mask expansion modelled. Both are blocked on the same library question above if they need footprint geometry - check that first.
