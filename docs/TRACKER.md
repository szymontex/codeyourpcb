...
- **Gate status, measured 2026-08-05:** 1 fmt PASS, 2 clippy PASS, 3 cargo test PASS, 4 eslint PASS, 5 vitest PASS, 6 playwright running, 7 autorouter benchmark PASS, 8 jscpd PASS.
- NEXT-ACTION: stage 6, Playwright. Chromium 1208/1217 are installed in the container. The question that matters is whether the WASM specs actually run or silently skip through `isWasmAvailable()` - that skip is the open caveat in `M005-VALIDATION.md`.
