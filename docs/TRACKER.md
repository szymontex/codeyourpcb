...
- NEXT-ACTION: an instance has no placement, so both dividers land on top of each other - `cypcb check` on the two-instance board reports 8 violations that are all "these parts are in the same spot". `use Divider as DIV1 at 10mm, 5mm` would offset the copies. That is the next thing the syntax needs, and it is grammar work again.
