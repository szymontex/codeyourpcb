...
- NEXT-ACTION: `project-manager.spec.ts` is the biggest remaining cluster at 8 failures and has nothing to do with routing. Probe the same way - load the app, dump what the spec waits for - before touching code.
- QUEUED: 6 failures come from `locator('.monaco-editor')` matching two elements (Monaco keeps two models), a strict-mode violation in the specs rather than an app fault. `#theme-toggle` expectation in `ui-architecture`. Three `benchmark-screenshots` failures that route boards through the hidden UI.
