...
- **Gate status, measured 2026-08-05:** 1 fmt PASS, 2 clippy PASS, 3 cargo test PASS, 4 eslint PASS, 5 vitest **FAIL**, 6 playwright not yet run, 7 autorouter benchmark PASS, 8 jscpd PASS.
- NEXT-ACTION: the 3 vitest failures, all pre-existing (same three fail on a clean HEAD, verified with `git stash`). `render-config.test.ts` expects violation colour `#C83434` and gets `#C41E1E`; `routing.test.ts` expects angle snap off by default and it is on, which also breaks `toggleAngleSnap flips state`. Decide which side is right - the constant or the test - for each.
- QUEUED: stage 6, Playwright. Chromium 1208/1217 are installed in the container. Run it and see whether the WASM specs skip or pass.
