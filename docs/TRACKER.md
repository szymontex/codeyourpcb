...
- NEXT-ACTION: **port `checkSilkClearance` to `silk_clearance.rs`**, which is V1's last stub and this vector's next blocker at the same time - the JS copy is the reason the viewer still owns DRC output. KNOWLEDGE.md K014 records that the check lives in JS; the geometry to copy is in `viewer/src/wasm.ts` around line 99. Then `checkTraceCurrentViolations`, then the swap itself: `build-wasm.sh` to `--features native`, the adapter onto `load_source`/`get_snapshot`, `registerDynamicFootprint` and `register3DModel` forwarding to the engine, `parseSource` and the mock's copy deleted in the same commit.
