...
- NEXT-ACTION: do the swap. `build-wasm.sh` to `--features native`, `WasmPcbEngineAdapter.load_source` calls the engine's `load_source` then `get_snapshot()`, `registerDynamicFootprint` forwards to `engine.register_footprint`, delete `parseSource` and its helpers. One commit - flipping the build alone ships both parsers. Check what else the adapter does that the engine does not: `regenerateRatsnest` and the net-constraint cache both run on the JS side today.
