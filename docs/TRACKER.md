...
|---|---|---|---|
| M1 | ~~Per-net electrical constraints on the board model.~~ **Closed**, and the router obeys them: a net that states a width is routed at it. | - | Per-net *clearance* is still ignored - the routing grid is built from one preset for the whole board, so a net asking for more space does not get it. |
