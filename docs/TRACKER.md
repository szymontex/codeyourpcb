...
- DONE: nothing this cycle.
- NEXT-ACTION: fresh clone does not build. `crates/cypcb-parser/grammar/src/parser.c` is gitignored (`.gitignore:19`) and `build.rs` panics without it. Either generate it in `build.rs` via the tree-sitter CLI, or commit the generated file. Affects every new contributor on every platform.
- QUEUED: `cargo fmt` drift in `crates/cypcb-cli/src/commands/parse_kicad.rs:16`. WASM-dependent Playwright tests only ever skip (`isWasmAvailable()` guard) - `M005-VALIDATION.md` verdict is `needs-attention` for exactly this. CI needs a `wasm-pack build` step before Playwright so the 6 WASM tests actually run. `cypcb check` + `cypcb export` in GitHub Actions (README lists it as Planned).
