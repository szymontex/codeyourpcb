...
- **Gate status, measured 2026-08-05:** stage 6 playwright **86 passed / 11 failed / 14 skipped** (0 attempted this morning, then 68/43, 66/31, 73/24).
- NEXT-ACTION: the last 11. Three are `benchmark-screenshots` blocked on D5 (they route through the hidden Route UI). The rest spread across `jlcpcb-search`, `reliability`, `theme`, `routing-ux`, `app-load` and `performance` - probe each cluster the same way rather than reading code first.
