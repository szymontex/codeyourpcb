...
- NEXT-ACTION: **the swap.** Every capability the viewer's board model has that the engine lacked is now in the engine: runtime footprints, 3D models, traces in the `load_source` snapshot, silkscreen clearance and trace current. `build-wasm.sh` to `--features native`, the adapter onto `load_source`/`get_snapshot`, `registerDynamicFootprint` and `register3DModel` forwarding to the engine, `parseSource`, `checkSilkClearance`, `checkTraceCurrentViolations` and the mock's parser deleted in one commit. Silk artwork from a fetch stays a known gap, M6.
