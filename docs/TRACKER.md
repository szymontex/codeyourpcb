...
- NEXT-ACTION: two stubs left and both now have the geometry they need. `silk_clearance.rs:26` wants silkscreen outlines - the renderer computes them from the same footprints. `solder_mask_bridge.rs:27` wants per-pad mask expansion, which `DesignRules.solder_mask_expansion` already carries.
