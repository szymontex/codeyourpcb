...
- **Gate status, measured 2026-08-05:** stage 6 playwright **90 passed / 7 failed / 14 skipped** (0 attempted this morning, then 68/43, 66/31, 73/24, 86/11).
- NEXT-ACTION: the last 7 split three ways. Three `benchmark-screenshots` are blocked on D5. Three `jlcpcb-search` tests expect 3 results and get 0 - decide whether they should hit the live LCSC API at all or be mocked, because a gate that depends on a third party is not a gate. One `project-manager` recent-files test remains.
