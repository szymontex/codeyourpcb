...
- NEXT-ACTION: retry the post-route repair pass now that its input is stable. The experiment above measured 115/135/129 on stm32_breakout precisely because the cells it forbade came from a report that moved between runs; with a reproducible report the same three runs must agree, and only then is the number worth believing.
