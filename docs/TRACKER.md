...
- NEXT-ACTION: R107 "zero DRC violations from the autorouter" - `routed_output_passes_drc` now passes, so measure the real violation count on all 3 benchmark fixtures and see whether the 5 grid-boundary artifacts noted in PROJECT.md survived this fix.
