...
- NEXT-ACTION: a module cannot contain another module. `expand_module_instances` copies a module's definitions once and does not look for instances inside them, so `use` nested in a `module` body is passed through untouched. Either expand recursively with a depth limit and a cycle check, or reject it with an error that says so - passing it through silently is the one option that is wrong.
