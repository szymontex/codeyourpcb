...
- NEXT-ACTION: the editor panel is the next cluster. Both remaining `project-manager` failures and the 3 in `editor.spec.ts` come down to `#editor-container` never becoming visible, and 6 more specs trip `locator('.monaco-editor')` matching two elements because Monaco keeps two models. Probe whether the panel opens at all before deciding which side is wrong.
