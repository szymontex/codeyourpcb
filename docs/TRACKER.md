...
- **Gate status, measured 2026-08-05:** stage 6 playwright **92 passed / 5 failed / 14 skipped** (0 attempted this morning, then 68/43, 66/31, 73/24, 86/11, 90/7).
- NEXT-ACTION: three of the last five are `benchmark-screenshots`, blocked on D5. Of the two that are not: `jlcpcb-search` 3D model pipeline never sees its easyeda fetch, and `project-manager` recent-files still times out. Probe the 3D one first - it exercises a real user path (click a part, get a model).
