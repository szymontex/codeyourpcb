...
| KiCad `.kicad_pcb` export | Done — `cypcb to-kicad` writes the outline, a footprint per part with its pads, nets and the face it is soldered to, every trace and via, and the net list. Zones, silkscreen text and 3D models are not written; KiCad fills its own defaults for what a file leaves out |
