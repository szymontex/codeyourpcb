...
| KiCad `.kicad_pcb` import to the language | Done — `cypcb from-kicad <board>.kicad_pcb` writes a `.cypcb` design: the board, a declared outline when it is not a plain rectangle, a `footprint` definition per part KiCad names, one `component` each with its value, placement, rotation and side, a `net` block per net, and the routed copper. The command re-reads what it wrote before reporting success. What a part *is* comes from its reference designator prefix, because KiCad does not record it, and `generic` where the prefix says nothing. A copper pour has no syntax in the language yet, so a board carrying one gets a comment counting what was dropped rather than losing it silently; a board whose pads are named rather than numbered - a USB-C receptacle's A1, B4, S1 - comes back carrying those names, and a net called `VBUS+` or `D-` comes back quoted, because the language takes both and renaming either would move pins onto the wrong nets |
