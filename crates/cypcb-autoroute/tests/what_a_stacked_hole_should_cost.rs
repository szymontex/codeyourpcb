...
//!
//! `cargo test --release -p cypcb-autoroute --test what_a_stacked_hole_should_cost -- --ignored --nocapture`
//!
...
//!
//! The map now counts how many vias pass through each cell, apart from the
//! rings, and the search adds `via_stack_penalty` per hole when it decides to
//! change layer there. Verified before this sweep was trusted, which the
//! fourth attempt did not do: routing `multi_ic` leaves **364 holes** recorded
//! for its 119 vias, and the penalty reaches the map the search reads.
//!
//! This measures what a price does, on both counts that matter: the stacked
//! holes it removes and the violations it costs.
...
fn what_a_stacked_hole_should_cost() {
    for name in [
        ...
        for penalty in [0.0, 5.0, 20.0, 100.0] {
            ...
            eprintln!(
                "  stack {penalty:>5}: {:>4} violations, {:>3} shorts, {:>3} hole-to-hole, \
                 {:>4} vias, {:>4} routes, {:.1}s",
