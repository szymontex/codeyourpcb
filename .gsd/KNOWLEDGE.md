...
**Still violated in one place, re-checked 2026-08-31:** `RoutingGrid::populate_pads` (`crates/cypcb-autoroute/src/grid.rs:274`) marks every pad as a circle of `max(w, h) / 2` - `let pad_radius_nm = pad.size.0.raw().max(pad.size.1.raw()) / 2;`. The checker uses rotated bounds, the router does not, and the two therefore disagree about where copper is. The router's error is the safe direction - it treats a pad as larger than it is, so it refuses gaps that are legal rather than routing through copper - which is why this has survived: it costs routes, not boards.
