...
- **"Quality gate script runs 6 stages": the gate runs 18 stages as of
  2026-09-14** - `grep -c '^echo "\[' scripts/quality-gate.sh`. The
  eighteenth is `cargo check --all-features`, added because five of the
  eleven features this workspace declares were switched on by nothing, and
  one of them gates production code that calls `reqwest`. The six named
  here are all still there, and nine ran up behind them: `tsc --noEmit` after
  ...
  comments that no test reads back, the exported values in the viewer that
  nothing names, a selftest of the scheduled gate after `playwright`, a
  check that no tracked file names a private path, a check that no number
  in a file this project writes comes from nowhere, and a check that every
  length reaching a Gerber or drill file goes through the one helper that knows
  the format. The count is held by
...
- **D-DRC-005, 12 rules with 2 stubs: superseded.** **40 rules are registered
  as of 2026-09-23** and none is a stub -
