...
class MockPcbEngine implements PcbEngine {
  ...
  load_source(source: string): string {
    ...
    // Add sample traces and ratsnest for visual testing
    this.addSampleTraces();
    this.addSampleRatsnest();
   ...
   * Add sample traces for visual testing of trace rendering.
   */
  private addSampleTraces(): void {
    // Only add if we have components
    if (this.snapshot.components.length < 2) return;

    const r1 = this.snapshot.components.find(c => c.refdes === 'R1');
    const c1 = this.snapshot.components.find(c => c.refdes === 'C1');
   ...
   * Add sample ratsnest lines for nets that aren't fully routed.
   */
  private addSampleRatsnest(): void {
    // For nets with 2+ connections, show ratsnest if no traces
